...
class ToDoEntity with _$ToDoEntity {
  ...
  Either<String, Unit> get _todoEmptyErrorMsg {
    return FormValidator.emptyValidator(text).fold(
      (failure) => failure.maybeWhen(orElse: () => right(unit), empty: () => left('To Do cannot be empty')),
      (_) => right(unit),
    );
  }
  ...
  Either<String, Unit> get _todoRedundantErrorMsg {
    return FormValidator.customValidator(validator: _cannotAddTheSameText).fold(
      (failure) => failure.maybeWhen(orElse: () => right(unit), costumError: () => left('To Do already exist')),
      (_) => right(unit),
    );
  }
  ...
  Either<FormFailure, Unit> addHistory() {
    return FormValidator.emptyValidator(text)
        .andThen(() => FormValidator.customValidator(validator: _cannotAddTheSameText))
        .fold(
      (failure) => left(failure),
      (_) {
        histories.add(ToDoHistoryEntity.newEntity(text: text));
        return right(unit);
      },
    );
  }
  ...
  List<ToDoHistoryEntity> updateHistories({int? id, String? text, bool? isDone}) {
    final newHistories = [...histories];
